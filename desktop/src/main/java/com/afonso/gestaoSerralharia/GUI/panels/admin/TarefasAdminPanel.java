...
public class TarefasAdminPanel extends BasePanel {
    ...
    private void abrirDialogoNova() {
        ...
        if (obras.isEmpty()) {
            mostrarErro(null, "Não existem obras disponíveis. Crie uma obra antes de registar tarefas.");
            return;
        }
        if (equipas.isEmpty()) {
            mostrarErro(null, "Não existem equipas disponíveis. Crie uma equipa associada a uma obra antes de registar tarefas.");
            return;
        }
    ...
    private class FormTarefa extends JPanel {
        ...
        private void preencherCombos() {
            for (Equipa equipa : equipas) cmbEquipa.addItem(new ItemEquipa(equipa, equipaLabel(equipa)));
            for (Estadotarefa estado : estados)
        ...
        private void atualizarFuncionariosDaEquipa() {
            ...
            if (itemEquipa == null || itemEquipa.equipa == null) {
                for (Funcionario funcionario : funcionarios) {
                    cmbFuncionario.addItem(new ItemFuncionario(funcionario, funcionarioLabel(funcionario)));
                }
                return;
            }
            ...
            // Fallback defensivo: se a equipa não tiver membros devolvidos, permite seleção manual global.
            for (Funcionario funcionario : funcionarios) {
                cmbFuncionario.addItem(new ItemFuncionario(funcionario, funcionarioLabel(funcionario)));
            }
        }
        ...
        private Tarefa construir() {
            ...
            ItemEquipa equipaSelecionada = (ItemEquipa) cmbEquipa.getSelectedItem();
            tarefa.setIdEquipa(equipaSelecionada.equipa);
            tarefa.setIdObra(equipaSelecionada.equipa != null ? equipaSelecionada.equipa.getIdObra() : null);
    ...
    private void carregarLookupsFormulario() {
        obras = obraService.listarTodos().stream()
                .sorted(Comparator.comparing(this::obraLabel, String.CASE_INSENSITIVE_ORDER))
                .collect(Collectors.toList());
        funcionarios = funcionarioService.listarTodos().stream()
